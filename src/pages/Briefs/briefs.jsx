...
const Briefs = () => {
    ...
    const cleraFilter = async () => {
        ...
        setIsLoading(true)
        axiosInstance.post("/api/briefs?limit=" + limit + "&page=" + page, {})
            .then((res) => {
    ...
    const getData = () => {
        setIsLoading(true)
        axiosInstance.post("/api/briefs?limit=" + limit + "&page=" + page +
            (filter?.year ? "&year=" + +filter?.year : "") + (filter?.client ? "&customer=" + filter?.client : "") + (filter?.manager ? "&employee=" + filter?.manager : ""), {})
            .then((res) => {
                console.log(res.data?.data)
                setIsLoading(false)
                setBriefs(res.data?.data)
            ...
            <div className="bg-white p-10 border-[1px] border-solid border-[#EBEBEB] rounded-[10px]">
                ...
                <div className="flex justify-between items-center">
                    <div className="flex w-full gap-4 justify-start">
                        <Select value={filter?.year} onChange={(value) => setFilter({ ...filter, year: value?.Name })} defOpt={filtersData.year.defOpt} options={filtersData.year.options} />
                        <Select value={filter?.client} onChange={(value) => setFilter({ ...filter, client: value?.Oid })} defOpt={filtersData.client.defOpt} options={filtersData.client.options} />
                        <Select value={filter?.manager} onChange={(value) => setFilter({ ...filter, manager: value?.Oid })} defOpt={managersData.manager.defOpt} options={managersData.manager.options} />
                    ...
                    <div className="flex gap-3">
                        ...
                        <button className={`${limit == 10 && "underline "} text-[14px] font-normal leading-[1.3] text-[#808080] cursor-pointer`} onClick={() => { setLimit(10); setPage(1) }}>10</button>
                        <button className={`${limit == 50 && "underline "} text-[14px] font-normal leading-[1.3] text-[#808080] cursor-pointer`} onClick={() => { setLimit(50); setPage(1) }}>50</button>
                        <button className={`${limit == 100 && "underline "} text-[14px] font-normal leading-[1.3] text-[#808080] cursor-pointer`} onClick={() => { setLimit(100); setPage(1) }}> 100</button>
                    </div>
                </div>
                <div className="mt-5 mb-10">
                    <button onClick={() => getData()} className="mr-5 text-[14px] font-semibold leading-[1.3] text-[#1A1A1A] cursor-pointer" >Применить выбранные</button>
                    <button className="text-[14px] font-normal leading-[1.3] text-[#808080] cursor-pointer" onClick={() => { cleraFilter(); }}>Отменить фильтры</button>
                </div>
                {
                    isLoading ?
                        ...
                        : <div className="min-w-[80vh]">
                            ...
                            <Pagination onChange={(e) => { setPage(e) }} meta={metaBrief} />
                        </div>}
            </div>
        </div>
    </div>

...
const FileUpload = () => {
    ...
    const getDesigners = () => {
        axiosInstance.post("/api/employees").then((res) => {
    ...
    const handleFile = (e) => {
        ...
        array.push({ file: e.target.files[0], brief: id, designer: "", approveDesigner: false, isLoading: false })
        setFileList([...array]);
    }
    ...
    const UploadAllFileToServer = async () => {
        ...
        if (isOk) {
            fileList?.map(async (sendDataFile) => {
                ...
                await axiosInstance.post("/api/briefs/files/add", formData).then((res) => {
    ...
    return <div className="file_form max-w-[450px] m-auto border-[1px] border-solid border-[#EBEBEB] rounded-[10px]">
        <div className="file_form-inner py-[30px] px-[50px] bg-white">
            {
                fileList?.map((item, i) => {
                    return <div key={"files" + i} className="file_item border-l-0 border-t-0 border-r-0 border-b border-solid border-[#EBEBEB] pb-5 mb-5">
                            ...
                            <div className="file_load-group ">
                                <div className="file_load-row flex items-start justify-between">
                                    ...
                                    <p className="file_load-size text-[16px] font-normal leading-[1.3] text-[#808080]">
                                        {item?.file && ((item?.file?.size) / 1024 > 1000 ? ((item?.file?.size) / 1024 / 1024).toFixed(2) + "MB" : ((item?.file?.size) / 1024).toFixed(2) + "KB")}
                                    </p>
                                </div>

                                <span className="file_load-progress block w-full h-1 bg-[#EBEBEB] rounded-[2px] mt-[5px]"></span>
                            </div>
                        </div>

                        <div className="file_select mt-[10px] ml-[30px] block">
                            <div onClick={() => deletUpFile(i)} className="cursor-pointer flex items-center mt-2 mb-4">
                            ...
                            <Select isDone={item?.designer?.approveDesigner} onChange={(e, indexOfFile) => addDesigner(e, indexOfFile)} options={designers} value={item?.designer} indexOfFile={i} />

                            {isDone && !item?.designer?.Oid ?
                                <span className={` file_select-alert block text-[14px] font-normal leading-[1.3] text-[#EB2F2F] mt-[16px]`}>
                                    !  Выберете дизайнера
                                </span>
                                : <div></div>
                            }

                            <div onClick={() => addApproved(i)} className="flex  items-center justify-between w-full mt-3" >
                                <p className={`${(item?.designer?.Oid ? "!cursor-pointer opacity-100" : " !pointer-events-none opacity-50") + " transition"}  file_select-btn text-[16px] font-normal leading-[1.3] block bg-transparent `}>
                                    Подтвердить
                                </p>

                                <span className={`${(item?.designer?.approveDesigner ? "" : "hidden") + " transition"} w-[20px] h-[20px] block`}>
                                    <svg width="12" height="9" viewBox="0 0 12 9" fill="none" xmlns="http://www.w3.org/2000/svg" className='block w-full h-full object-contain'>
                                        <path d="M4.00005 6.60005L9.90005 0.700048C10.0834 0.516715 10.3167 0.425049 10.6 0.425049C10.8834 0.425049 11.1167 0.516715 11.3 0.700048C11.4834 0.883382 11.575 1.11671 11.575 1.40005C11.575 1.68338 11.4834 1.91672 11.3 2.10005L4.70005 8.70005C4.50005 8.90005 4.26672 9.00005 4.00005 9.00005C3.73338 9.00005 3.50005 8.90005 3.30005 8.70005L0.700048 6.10005C0.516715 5.91672 0.425049 5.68338 0.425049 5.40005C0.425049 5.11671 0.516715 4.88338 0.700048 4.70005C0.883382 4.51672 1.11672 4.42505 1.40005 4.42505C1.68338 4.42505 1.91672 4.51672 2.10005 4.70005L4.00005 6.60005Z" fill="#1E7E2E" />
                                    </svg>
                                </span>
                            </div>
                        </div>
            ...
            <input onChange={(e) => handleFile(e)} ref={fileRef} accept="image/*, .pdf" type="file" className="hidden" />

            <button onClick={() => { fileRef.current.click(); }} className="addMore mt-[10px] ml-[30px] bg-transparent cursor-pointer flex items-center text-[14px] font-normal leading-[1.3]">
        ...
        <button onClick={() => UploadAllFileToServer()} className=" text-[18px] font-normal leading-[1.3] text-center w-full py-5 px-[10px] cursor-pointer bg-[#F5F5F5]">
            Готово
        </button>
    </div>
}

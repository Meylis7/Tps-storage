        ...
        <div className='auto_container'>
            <div className='w-full mt-[20px] flex items-center justify-center'>
                ...
                <ul className='flex items-center border-[1px] border-solid border-[#EBEBEB] rounded-[5px]'>
                    ...
                    {
                        array?.map((item, i) => {
                            return ((item != 1 && item != props?.meta?.last_page) && ((Math.abs(currentPage - item) <= 2 && Math.abs(currentPage - item) != 0) || currentPage == item)) && <li key={"pages" + i} className='block'>
                                <div onClick={() => setCurentPage(item)} className={`${currentPage == item && "bg-red-600 text-white "} flex items-center justify-center hover:bg-[#EBEBEB] text-black cursor-pointer w-[38px] h-[38px]`}>
                                    {item}
                                </div>
                            </li>
                        })
